  ...
  return (
    <div className="space-y-6 text-left">
      ...
      {loading ? (
        ...
        <p className="text-xs text-neutral-400 py-10 text-center">{lang === "en" ? "No matches found." : "Hakuna waliopatikana."}</p>
      ) : (
          ...
          {filtered.map(p => (
            <div key={p.id} className="bg-white border border-neutral-200 rounded-xl p-4 text-center space-y-2 shadow-2xs">
              <div className="w-14 h-14 mx-auto rounded-full bg-neutral-100 border border-neutral-200 overflow-hidden flex items-center justify-center">
                {p.avatar ? <img src={p.avatar} alt={p.name} className="w-full h-full object-cover" referrerPolicy="no-referrer" /> : <span className="text-neutral-400 font-bold text-lg">{p.name?.[0]}</span>}
              </div>
              <div>
                <p className="text-xs font-bold text-neutral-900 truncate">{p.name}</p>
                <p className="text-[10px] text-neutral-400 font-mono truncate">{p.role}</p>
              </div>
